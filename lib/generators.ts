...
import { attributeValidations, resolveRef, deriveSwaggerTypeFromExample } from './utils';
      ...
      if (route.actionType === 'actions2') {
        ...
        if (route.actions2Machine?.inputs) {
          forEach(route.actions2Machine.inputs, (value, key) => {
            ...
            if(!attribute.type && 'example' in attribute) { // derive type if not specified (optional for action2)
